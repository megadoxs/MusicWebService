...
original_duration=$(echo "$get_body" | jq -r '.duration')
echo "ℹ️ Original duration: $original_duration"

echo "✏️ Updating playlist (remove one song)"
...
# Check new duration
get_updated=$(curl -s "$API_URL/$playlist_id")
new_duration=$(echo "$get_updated" | jq -r '.duration')
echo "ℹ️ Updated duration: $new_duration"

if [ "$original_duration" == "$new_duration" ]; then
  echo "❌ Duration invariant failed: duration did not change after removing a song"
  exit 1
else
  echo "✅ Duration invariant maintained: duration updated correctly"
fi
...
echo "🎉 All CRUD and duration invariant tests passed successfully."
